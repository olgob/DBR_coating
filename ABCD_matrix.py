...
# def transfer_matrix_method(stack, a0, b0, lam, theta):
#     """
...
#             theta_tot (array): Accumulated angle thought the stack.
#
#     """
#
...
#     index_tot = 0
#     theta_tot = 0
#     matrix_te = np.zeros([2, 2, len(n)], dtype=complex)
#     matrix_tm = np.zeros([2, 2, len(n)], dtype=complex)
#     matrix_prop = np.zeros([2, 2, len(n)], dtype=complex)
#
#     # Begin of the calculation
#     for i, re_index in enumerate(n):
#         step_size = length.sum() / 10000
#         z0 = np.linspace(0, length[i], round(length[i] / step_size))
#         if i > 0:
#             # The angle of the beam is modified by refraction (Descartes law)
#             theta = np.arcsin(np.real(re_index) / np.real(n[i - 1]) * sind(theta)) * 180 / np.pi
#
#         # Matrices corresponding to the propagation though the interface i for TE and TM polarized light
#         matrix_te[:, :, i] = np.array([[1, 1], [np.real(re_index) * cosd(theta), -np.real(re_index) * cosd(theta)]])
#         matrix_tm[:, :, i] = np.array([[cosd(theta), cosd(theta)], [np.real(re_index), -np.real(re_index)]])
#
#         # Matrix corresponding to the propagation within the layer i
#         phi = (2 * np.pi / lam) * re_index * length[i] * cosd(theta)
#         matrix_prop[:, :, i] = np.array([[np.exp(1j * phi), 0], [0, np.exp(-1j * phi)]], dtype=complex)
#
#         # We use ABCD matrices to calculate the amplitudes of TE and TM polarized light in layer i
#         if i == 0:
#             amp_te = amp_te
#             amp_tm = amp_tm
#         else:
...
#                 np.dot(
#                     np.dot(
#                         np.linalg.inv(matrix_te[:, :, i]), matrix_te[:, :, i - 1]),
#                     matrix_prop[:, :, i - 1]),
#                 amp_te)
#             amp_tm = np.dot(
#                 np.dot(
#                     np.dot(
#                         np.linalg.inv(matrix_tm[:, :, i]), matrix_tm[:, :, i - 1]),
#                     matrix_prop[:, :, i - 1]),
#                 amp_tm)
#
#         # Calculation of the electric field of TE and TM polarized light (standing waves)
#         k = (2 * np.pi / lam) * re_index * cosd(theta)  # Wave vector
#         electric_te = amp_te[0] * np.exp(1j * k * z0) + amp_te[1] * np.exp(-1j * k * z0)
#         electric_tm = amp_tm[0] * np.exp(1j * k * z0) + amp_tm[1] * np.exp(-1j * k * z0)
#         # We add the values of the electric field to the previous ones
#         electric_tot_te = np.hstack((electric_tot_te, electric_te))
#         electric_tot_tm = np.hstack((electric_tot_tm, electric_tm))
#         # We add the values of the index to the previous ones
#         n_length_prod = re_index + 0 * z0
#         index_tot = np.hstack((index_tot, n_length_prod))
#         # We add the values of theta to the previous ones
...
#     transmission_tot_te = np.abs(a0 / amp_te[0]) ** 2
#     transmission_tot_tm = np.abs(a0 / amp_tm[0]) ** 2
#
